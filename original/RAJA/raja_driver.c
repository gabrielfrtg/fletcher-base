...
{
	RAJA_Update_pointers(sx,sy,sz,pc);
}
	              ...
	              float * pp, float * pc, float * qp, float * qc)
{
	...
	// RAJA_Propagate also does TimeForward
	   RAJA_Propagate(  sx,   sy,   sz,   bord,
	                    dx,   dy,   dz,   dt,   it,
	                    pp,    pc,    qp,    qc);
...
{
	RAJA_InsertSource(src, iSource, p, q);
}
